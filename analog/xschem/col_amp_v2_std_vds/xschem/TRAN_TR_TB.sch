...
C {code_shown.sym} 420 -110 0 0 {name=TRAN 
only_toplevel=false 
place=end
value=
"
...
.option trtol=1 abstol=1p chgtol=4e-16 vntol=1e-6 reltol=1e-6
.option temp = 27

.save v(COL_OUT) tset_col tr_col_out nosub nointernals
...
.control
	...
	*let rise_time_col_out vector(mc_runs)
	*reshape rise_time_col_out [mc_runs]

	*let settling_time_col_out vector(mc_runs)
	*reshape settling_time_col_out [mc_runs]
	
	*save v(column_out) v(nCHARGE) v(COL_OUT) v(ON) v(nOFF) v(READON) v(READOFF) tset_col tr_col_out nosub nointernals
	save none 
	
	op
	write TRAN_TR_TB.raw 

	set appendwrite
	dowhile run <= mc_runs
		
		*save v(COL_OUT) rise_time_col_out settling_time_col_out nosub nointernals
		save v(COL_OUT) tset_col tr_col_out nosub nointernals
		...
		*meas tran tf_col_out TRIG v(COL_OUT) VAL=Vc_90 CROSS=2 TARG v(COL_OUT) VAL=Vc_10 CROSS=2
		meas tran tset_col_low_trig WHEN v(COL_OUT)=Vc_set_low RISE=LAST
		
		let tset_col_high_trig = tset_col_low_trig
		meas tran tset_col_high_trig WHEN v(COL_OUT)=Vc_set_high FALL=LAST

		let tset_col = ((tset_col_low_trig + tset_col_high_trig)/2) - start_pulse
		
		write TRAN_TR_TB.raw tset_col tr_col_out
		let run = run + 1
		reset
